...
$execute at @e[type=minecraft:marker,tag=BingoMastermind_$(instance)_Start,limit=1] as @a[gamemode=adventure,team=$(teamName),tag=BingoModule_$(caseName)_Playing,scores={BingoMMCooldown=0,BingoMMState=0},distance=..40,sort=nearest,limit=1] run function sb:module/mastermind/player_start {instance:"$(instance)",teamName:"$(teamName)",caseName:"$(caseName)"}

# Les laines casseees redeviennent des items vanilla : restaure leur restriction de pose localement.
$execute at @e[type=minecraft:marker,tag=BingoMastermind_$(instance)_Start,limit=1] run function sb:module/mastermind/restore_dropped_wool
