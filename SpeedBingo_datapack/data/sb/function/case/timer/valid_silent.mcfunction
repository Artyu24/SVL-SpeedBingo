...
$execute if entity @e[type=minecraft:marker,tag=$(caseName),tag=!$(teamWin)Win,tag=!$(teamLose)Win,limit=1] run return run function sb:case/timer/valid_noteam {teamWin:"$(teamWin)",teamLose:"$(teamLose)",caseName:"$(caseName)",colorBlock:"$(colorBlock)",colorText:"$(colorText)"}
